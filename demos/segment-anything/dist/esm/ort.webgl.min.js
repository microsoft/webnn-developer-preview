 ...
 ${f}`,f=f.replace("return",`${i} = `);let d=`
    ...
    `),t="packedUVfrom1D",o[t]=new I(`
      ...
      `),t="sampleTexture";let n=D(this.context.glContext.version);return o[t]=new I(`
        ...
        }`),o}getInputsSamplingSnippets(){let o={},t=this.context.outputTextureLayout;return this.context.programInfo.inputNames.forEach((n,r)=>{let s=this.context.inputTextureLayouts[r],i=rr(n);s.isPacked?o[i]=this.getPackedSamplerFromInput(i,n,s):o[i]=this.getUnpackedSamplerFromInput(i,n,s);let u=Ui(n);s.unpackedShape.length<=t.unpackedShape.length&&(s.isPacked?o[u]=this.getPackedSamplerAtOutputCoords(u,s,t,n):o[u]=this.getUnpackedSamplerAtOutputCoords(u,s,t,n))}),o}getPackedSamplerAtOutputCoords(o,t,n,r){let s=t.unpackedShape,i=n.unpackedShape,l=rr(r),f=s.length,p=i.length,d=it.getBroadcastDims(s,i),b=at(p),g=p-f,y,v=_t();f===0?y="":p<2&&d.length>=1?y="coords = 0;":y=d.map(Mt=>`coords.${v[Mt+g]} = 0;`).join(`
`);let _="";p<2&&f>0?_="coords":_=s.map((Mt,$e)=>`coords.${v[$e+g]}`).join(", ");let A="return outputValue;",C=O.size(s)===1,st=O.size(i)===1;if(f===1&&!C&&!st)A=`
      ...
      `;else if(C&&!st)p===1?A=`
        ...
        `;else if(d.length){let Mt=f-2,$e=f-1;d.indexOf(Mt)>-1&&d.indexOf($e)>-1?A="return vec4(outputValue.x);":d.indexOf(Mt)>-1?A="return vec4(outputValue.x, outputValue.y, outputValue.x, outputValue.y);":d.indexOf($e)>-1&&(A="return vec4(outputValue.xx, outputValue.zz);")}let St=`
