...
// built-in gradients
const PRISM = ["rgba(230, 230, 230, 1)"],
    GRADIENTS = [
        [
            "classic",
            {
                colorStops: ["red", { color: "yellow", level: 0.85, pos: 0.6 }, { color: "lime", level: 0.475 }],
            },
        ],
        [
            "prism",
...
export default class AudioMotionAnalyzer {
    ...
    _calcBars() {
        ...
        const {
            ...
            channelHeight = (canvas.height - (isDualVertical && !isLeds ? 0.5 : 0)) >> isDualVertical,
        ...
        let innerRadius =
                (Math.min(canvas.width, canvas.height) * 0.375 * (_chLayout == CHANNEL_VERTICAL ? 1 : this._radius)) |
                0,
		...
		*/
        ...
        if (isOctaves) {
            ...
            const roundSD = (value, digits, atLeast) =>
                +value.toPrecision(atLeast ? Math.max(digits, (1 + Math.log10(value)) | 0) : digits);
            ...
            do {
                ...
                if (freq >= _minFreq) barsPush({ posX: 0, freq, freqLo, freqHi, binLo, binHi, ratioLo, ratioHi });
    ...
    _createScales() {
        ...
        for (const item of freqLabels) {
            const [freq, label] = Array.isArray(item)
                ...
                isC = label[0] == "C",
                maxW = fontSizeX * (_noteLabels && !_mirror && !isDualHorizontal ? (isC ? 1.2 : 0.6) : 3);

            // set label color - no highlight when mirror effect is active (only Cs displayed)
            _scaleX.fillStyle = _scaleR.fillStyle =
                isC && !_mirror && !isDualHorizontal ? SCALEX_HIGHLIGHT_COLOR : SCALEX_LABEL_COLOR;
            ...
            if (x >= prevX + labelWidthX / 2 && x <= analyzerWidth) {
                ...
                if (isDualHorizontal || (_mirror && (x > labelWidthX || _mirror == 1)))
                    _scaleX.fillText(
                        label,
                        isDualHorizontal && _mirror != 1 ? analyzerWidth + x : (initialX || canvas.width) - x,
                        y,
                        maxW,
                    );
    ...
    _draw(timestamp) {
        ...
        const { isAlpha, isBands, isLeds, isLumi, isOctaves, isOutline, isRound, noLedGap } = this._flg,
            {
                analyzerHeight,
                centerX,
                centerY,
        ...
        for (let channel = 0; channel < nChannels; channel++) {
            const { channelTop, channelBottom, analyzerBottom } = channelCoords[channel],
                ...
                radialOffsetX =
                    !isDualHorizontal || (channel && _mirror != 1) ? 0 : analyzerWidth >> (channel || !invertedChannel),
...
*/
                    ...
                    const selectedIndex =
                        _colorMode == COLOR_BAR_INDEX
                            ...
                            : colorStops.findLastIndex(item =>
                                  isLeds ? ledPosY(value) <= ledPosY(item.level) : value <= item.level,
                              );
                ...
                if (!overlay || showBgColor) {
                    ...
                    if (channel == 0 || (!_radial && !isDualCombined))
                        _ctx.fillRect(
                            ...
                            analyzerWidth,
                            (overlay && this.reflexAlpha == 1 ? analyzerHeight : channelHeight) + channelGap,
                        );
            ...
            for (let barIndex = 0; barIndex < nBars; barIndex++) {
                ...
                if (_mode == MODE_GRAPH) {
                    ...
                    const nextBarAvg = barIndex
                        ? 0
                        : (this._normalizedB(fftData[_bars[1].binLo]) * maxBarHeight + barHeight) / 2;
                    ...
                    if (isLeds) {
                        ...
                        if (isTrueLeds) {
                            ...
                            const colorIndex = isLumi
                                ? 0
                                : colorStops.findLastIndex(item => ledPosY(barValue) <= ledPosY(item.level));
                ...
                if (peak > 0 && showPeaks && !showPeakLine && !isLumi && posX >= initialX && posX < finalX) {
                    ...
                    if (isLeds) {
                        ...
                        radialPoly(
                            ...
                            y,
                            width,
                            !this._radialInvert || isDualVertical || y + innerRadius >= 2 ? -2 : 2,
                        );
                    }
                }
            } // for ( let barIndex = 0; barIndex < nBars; barIndex++ )
        ...
        for (const channel of [0, 1]) {
            ...
            if (_radial)
                ...
                grad = _ctx.createLinearGradient(
                    ...(isHorizontal ? [initialX, 0, initialX + analyzerWidth, 0] : [0, 0, 0, gradientHeight]),
                );
    ...
    _setCanvas(reason) {
        ...
        const isFullscreen = this.isFullscreen,
            ...
            newWidth = isCanvasFs
                ? screenWidth
                : ((this._width || this._container.clientWidth || this._defaultWidth) * pixelRatio) | 0,
            newHeight = isCanvasFs
                ? screenHeight
                : ((this._height || this._container.clientHeight || this._defaultHeight) * pixelRatio) | 0;
    ...
    _setProps(options, useDefaults) {
        ...
        if (useDefaults || options === undefined) options = { ...DEFAULT_SETTINGS, ...options }; // merge options with defaults
