        ...
        for (let d = 0; d < 64; d++) {
          for (let s = 0; s < 4; s++) {
            decoder_input[`past_key_values.${i}.decoder.key`].cpuData[
              ...
              past_key_values[`present_key_values.${i}.decoder.key`].cpuData[
                h * 4 * 64 + s * 64 + d
              ];
            decoder_input[`past_key_values.${i}.decoder.value`].cpuData[
              h * 64 * 127 + s * 64 + d
            ] =
              past_key_values[`present_key_values.${i}.decoder.value`].cpuData[
                h * 4 * 64 + s * 64 + d
              ];
          }
        }
...
export function attention_mask_update(
  ...
  if (inf_iter === 0) {
    if (!mask_4d) {
      ...
      let padded_mask = new BigInt64Array(
        max_sequence_length - num_init_tokens - 1,
      ).fill(0n);
      // create a new attention mask array with the padded mask and the value 1 for the new token
      let updated_mask = new BigInt64Array(
        attention_mask.length + padded_mask.length + 1,
      );
      ...
      let padded_mask = new Uint16Array(
        max_sequence_length - num_init_tokens - 1,
      ).fill(toHalf(-65500));
      let updated_mask = new Uint16Array(
        attention_mask.length + padded_mask.length + 1,
      );
      updated_mask.set(attention_mask, 0);
      updated_mask.set(padded_mask, attention_mask.length);
