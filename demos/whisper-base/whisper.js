...
export class Whisper {
  ...
  async run(audio_data) {
    ...
    const decoder_output =
      await this.models['decoder']['sess'].run(decoder_input);
    ...
    for (let i = 4; i < this.max_sequence_length; i++) {
      ...
      // start = performance.now();
      const decoder_cached_output =
        await this.models['decoder_cached']['sess'].run(decoder_input);
